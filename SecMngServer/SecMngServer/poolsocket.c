...
int sckCltPool_init(void **handle, SCKClitPoolParam *param)
{
	...
	{
		ret = Sck_ErrMalloc;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func sckCltPool_init(), check malloc err");
		return ret;
	}
	...
	for (i = 0; i < hdl->bounds; i++)
	{
		...
		if (ret != 0)
		{
			ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func sckClient_connect() err");
			break;
		}
		else
		{
	...
	if (hdl->valid < hdl->bounds) //若有效连接数 小于 总数
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "有效连接数 小于 总数");
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func sckClient_init() create connect num err:%d,  hdl->valid: %d , hdl->bounds:%d", ret, hdl->valid, hdl->bounds);
		for (i = 0; i < hdl->bounds; i++)
		{
...
END:
	...
	if (ret != 0)
	{
		...
		free(hdl);
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func pthread_mutex_unlock() err");
		return ret;
	}
...
int sckCltPool_getConnet(void *handle, int *connfd)
{
	...
	{
		ret = Sck_ErrParam;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func sckCltPool_getConnet() (handle == NULL || connfd==NULL) err");
		return ret;
	}
	...
	if (hdl->terminated == 1)
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func sckCltPool_getConnet() (terminated == 1)");
		goto END;
	}
	...
	if (hdl->valid == 0)
	{
		...
		if (hdl->valid == 0)
		{
			...
			ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, "func sckCltPool_getConnet() Sck_Err_Pool_GetConn_ValidIsZero err");
			goto END;
		}
	...
	if (hdl->statusArray[hdl->valid - 1] == 0)
	{
		...
		if (hdl->fdArray[hdl->valid - 1] == 0)
		{
			...
			if (ret != 0)
			{
				ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckClient_closeconn() err 断开坏掉的连接失败");
		...
		if (ret != 0)
		{
			ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckClient_connect() err 断链修复 重新连接失败");
			hdl->fdArray[hdl->valid - 1] = 0;
			goto END;
		}
	}
...
int sckCltPool_send(void *handle, int  connfd, unsigned char *data, int datalen)
{
	...
	{
		ret = Sck_ErrParam;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_send() err (handle==NULL || connfd<0 || data==NULL || datalen<=0) ");
		return ret;
	}
	hdl = (SockePoolHandle *)handle;
	...
	if (ret != 0)
	{
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckClient_send() err");
		return ret;
	}
...
int sckCltPool_rev(void *handle, int  connfd, unsigned char **out, int *outlen)
{
	...
	{
		ret = Sck_ErrParam;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_rev() err, check (handle==NULL || connfd<0 || out==NULL || outlen==NULL )");
		return ret;
	}
	hdl = (SockePoolHandle *)handle;
	...
	if (ret != 0)
	{
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckClient_rev() err");
		return ret;
	}
...
int sckCltPool_putConnet(void *handle, int connfd, int validFlag)
{
	...
	{
		ret = Sck_ErrParam;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_putConnet() err, check (handle == NULL || connfd==NULL)");
		goto END;
	}
	...
	{
		ret = Sck_Err_Pool_terminated;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_putConnet() err, check (func sckCltPool_putConnet() (terminated == 1))");
	...
	for (i = 0; i < hdl->valid; i++)
	{
		if (hdl->fdArray[i] == connfd)
		{
			...
			ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_putConnet() err, check Sck_Err_Pool_HaveExist ");
			goto END;
		}
	}
	...
	if (hdl->valid >= hdl->bounds)
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_putConnet() err, check (hdl->valid >= hdl->bounds) ");
		goto END;
	}
	...
	else
	{
		...
		if (ret != 0)
		{
			ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckClient_closeconn() err, check (hdl->valid >= hdl->bounds) ");
			//失败不处理
		}
...
int sckCltPool_destroy(void *handle)
{
	...
	{
		ret = Sck_ErrParam;
		ITCAST_LOG(__FILE__, __LINE__, IC_ERROR_LEVEL, ret, " func sckCltPool_destroy() err, check (handle == NULL)");
		return ret;
	}
